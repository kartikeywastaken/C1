...
void position(struct user_data posi){ // to locate users position.
    for ( int i = 0; i < 10; i++)
    {
        for ( int j = 0; j < 10; j++)
        {
            if (j == posi.x_cor && i == posi.y_cor)
            {
                printf("@");
            } else {
...
int main(){
    ...
    struct user_data player1;
    {
        ...
        while (player1.user_health > 0 && player1.x_cor != 7 && player1.y_cor != 6)
        {
            ...
            if (player1.user_move == 'd')
            {
                ...
                position(player1); // trying to print the updated map 
            }
